...
INTERVIEW_PROMPT_PREFIX = "당신의 페르소나, 모든 과거 기억과 행동을 결합하여, 도구를 호출하지 말고 텍스트로 직접 답변해 주세요:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Interview 질문 최적화, 접두사를 추가하여 Agent의 도구 호출 방지
...
def get_graph_entities(graph_id: str):
    ...
    사전 정의된 엔티티 타입에 맞는 노드만 반환 (Labels가 Entity만 있는 노드 제외)
...
def create_simulation():
    ...
    참고: max_rounds 등 파라미터는 LLM이 자동 생성, 수동 설정 불필요
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    참고: 실행 스크립트(run_*.py)는 backend/scripts/ 디렉토리에 유지, 시뮬레이션 디렉토리에 복사 안함
    ...
    if not os.path.exists(simulation_dir):
        return False, {"reason": "시뮬레이션 디렉토리 없음"}
    ...
    try:
        ...
        # 다음 상태들은 모두 준비 작업 완료를 의미:
        # - ready: 준비 완료, 실행 가능
        # - preparing: config_generated=True면 이미 완료됨
        ...
        if status in prepared_statuses and config_generated:
            ...
            # 상태가 preparing이지만 파일이 완료되면 상태를 ready로 자동 업데이트
            if status == "preparing":
                try:
...
def prepare_simulation():
    """
    시뮬레이션 환경 준비 (비동기 작업, LLM이 모든 파라미터 자동 생성)

    시간이 소요되는 작업으로 API는 즉시 task_id를 반환하고
    GET /api/simulation/prepare/status로 진행 상황 조회
    ...
    4. LLM이 시뮬레이션 설정 자동 생성 (재시도 메커니즘 포함)
    ...
    요청 (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // 선택, LLM으로 페르소나 생성 여부
    ...
    반환:
        {
            ...
            "data": {
                ...
                "message": "준비 작업 시작됨|이미 완료된 준비 작업 존재",
    ...
    try:
        ...
        # ========== 엔티티 수 동기 조회 (백그라운드 작업 시작 전) ==========
        # 이렇게 하면 프론트엔드가 prepare 호출 후 즉시 예상 Agent 총 수를 알 수 있음
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # 엣지 정보 미조회로 속도 향상
            )
            # 엔티티 수를 상태에 저장 (프론트엔드에서 즉시 조회 가능)
            state.entities_count = filtered_preview.filtered_count
            state.entity_types = list(filtered_preview.entity_types)
            logger.info(f"예상 엔티티 수: {filtered_preview.filtered_count}, 타입: {filtered_preview.entity_types}")
        except Exception as e:
            logger.warning(f"엔티티 수 동기 조회 실패 (백그라운드 작업에서 재시도 예정): {e}")
            # 실패해도 후속 처리에 영향 없음, 백그라운드 작업에서 재조회
...
def get_prepare_status():
    ...
    요청 (JSON):
        {
            "task_id": "task_xxxx",          // 선택, prepare에서 ��환된 task_id
    ...
    try:
        ...
        if not task:
            # 작업 없음, 하지만 simulation_id가 있으면 준비 완료 여부 확인
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    reports 디렉토리를 순회하여 simulation_id가 일치하는 report 찾기
    ...
    # __file__은 app/api/simulation.py, 상위 두 단계로 backend/까지
...
def get_simulation_history():
    ...
    홈페이지 히스토리 프로젝트 표시용, 프로젝트 이름, 설명 등 풍부한 정보 포함
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "우한대학교에서 발표하면...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            # 관련 report_id 가져오기 (해당 simulation의 최신 report 찾기)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    시뮬레이션 Agent Profile 실시간 조회 (생성 중 실시간 진행 확인용)
...
def get_simulation_config_realtime(simulation_id: str):
    """
    시뮬레이션 설정 실시간 조회 (생성 중 실시간 진행 확인용)

    /config 인터페이스와의 차이점:
    - 파일 직접 읽기, SimulationManager 거치지 않음
    - 생성 과정 중 실시간 확인에 적합
    - 추가 메타데이터 반환 (파일 수정 시간, 생성 중 여부 등)
    - 설정이 아직 완성되지 않아도 일부 정보 반환 가능
...
def get_simulation_config(simulation_id: str):
    ...
    반환 내용:
        - time_config: 시간 설정 (시뮬레이션 시간, 라운드, 피크/오프피크 시간대)
...
def start_simulation():
    ...
    요청 (JSON):
        {
            ...
            "max_rounds": 100,                     // 선택: 최대 시뮬레이션 라운드, 너무 긴 시뮬레이션 절단용
    ...
    try:
        ...
        max_rounds = data.get('max_rounds')  # 선택: 최대 시뮬레이션 라운드
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # 프로세스 없거나 종료됨, 상태를 ready로 리셋
                logger.info(f"시뮬레이션 {simulation_id} 준비 작업 완료, 상태를 ready로 리셋 (이전 상태: {state.status.value})")
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # 현재 라운드 액션 가져오기 (recent_actions는 최신 라운드만 표시)
...
def get_agent_stats(simulation_id: str):
    ...
    프론트엔드 Agent 활성도 순위, 액션 분포 등 표시용
    """
    try:
...
def get_simulation_comments(simulation_id: str):
    """
    시뮬레이션 댓글 조회 (Reddit만 해당)

    Query 파라미터:
        post_id: 게시물 ID 필터 (선택)
        limit: 반환 수량
        offset: 오프셋
    """
    try:
...
def interview_agent():
    ...
    참고: 이 기능은 시뮬레이션 환경이 실행 중인 상태에서만 가능 (시뮬레이션 루프 완료 후 명령 대기 모드)
    ...
    try:
        ...
        # prompt 최적화, Agent 도구 호출 방지를 위해 접두사 추가
...
def interview_agents_batch():
    ...
    참고: 이 기능은 시뮬레이션 환경이 실행 중인 상태에서만 가능
    ...
    요청 (JSON):
        {
            ...
            "platform": "reddit",              // 선택, 기본 플랫폼 (각 항목의 platform으로 덮어쓰기됨)
                                               // 미지정 시: 양쪽 플랫폼에서 각 Agent 동시 인터뷰
    ...
    try:
        ...
        # 각 인터뷰 항목의 prompt 최적화, Agent 도구 호출 방지를 위해 접두사 추가
...
def interview_all_agents():
    ...
    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",            // 필수, 시뮬레이션 ID
            "prompt": "이 일에 대해 전체적으로 어떻게 생각하세요?",  // 필수, 인터뷰 질문 (모든 Agent 동일 질문 사용)
...
def get_interview_history():
    ...
    요청 (JSON):
        {
            ...
            "platform": "reddit",          // 선택, 플랫폼 타입 (reddit/twitter)
                                           // 미지정 시 양쪽 플랫폼의 모든 히스토리 반환
            "agent_id": 0,                 // 선택, 해당 Agent의 인터뷰 히스토리만 조회
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # 미지정 시 양쪽 플랫폼의 히스토리 반환
        agent_id = data.get('agent_id')
        ...
        logger.error(f"Interview 히스토리 조회 실패: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_env_status():
    ...
    시뮬레이션 환경이 살아있는지(Interview 명령 수신 가능 여부) 확인
    ...
    응답:
        {
            ...
            "data": {
                ...
                "message": "환경이 실행 중이며, Interview 명령 수신 가능"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    시뮬레이션에 환경 종료 명령을 전송하여 대기 모드에서 정상 종료되도록 함.

    참고: /stop 인터페이스와 다름. /stop은 프로세스를 강제 종료하지만,
    이 인터페이스는 시뮬레이션이 환경을 정상적으로 종료하고 나감.
