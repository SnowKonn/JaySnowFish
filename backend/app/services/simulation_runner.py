"""
OASIS 시뮬레이션 러너
백그라운드에서 시뮬레이션을 실행하고 각 에이전트의 동작을 기록, 실시간 상태 모니터링 지원
"""
...
class SimulationRunState:
    ...
    # 각 플랫폼 독립 라운드 및 시뮬레이션 시간 (이중 플랫폼 병렬 표시용)
...
class SimulationRunner:
    ...
    2. 실행 로그를 파싱하여 각 에이전트의 동작 기록
    3. 실시간 상태 쿼리 인터페이스 제공
    4. 일시중지/중지/재개 작업 지원
    """
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # 최대 시뮬레이션 라운드 (선택사항, 너무 긴 시뮬레이션 절삭용)
        ...
        Args:
            ...
            max_rounds: 최대 시뮬레이션 라운드 (선택사항, 너무 긴 시뮬레이션 절삭용)
            enable_graph_memory_update: 에이전트 활동을 Zep 그래프에 동적으로 업데이트할지 여부
        ...
        if max_rounds is not None and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                logger.info(f"라운드 절삭: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        try:
            ...
            # 메인 로그 파일 생성, stdout/stderr 파이프 버퍼가 가득 차서 프로세스가 차단되는 것을 방지
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # 하위 프로세스 환경 변수 설정, Windows에서 UTF-8 인코딩 사용 보장
            # 서드파티 라이브러리(예: OASIS)가 인코딩을 지정하지 않고 파일을 읽을 때의 문제 해결
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ 지원, 모든 open()이 기본적으로 UTF-8 사용
            env['PYTHONIOENCODING'] = 'utf-8'  # stdout/stderr이 UTF-8 사용하도록 보장

            # 작업 디렉토리를 시뮬레이션 디렉토리로 설정 (데이터베이스 등 파일이 여기에 생성됨)
            # start_new_session=True를 사용하여 새 프로세스 그룹 생성, os.killpg로 모든 하위 프로세스 종료 가능
            ...
            cls._stderr_files[simulation_id] = None  # 별도의 stderr이 더 이상 필요하지 않음
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            logger.error(f"모니터 스레드 예외: {simulation_id}, error={str(e)}")
            state.runner_status = RunnerStatus.FAILED
            state.error = str(e)
            cls._save_run_state(state)
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    logger.info(f"그래프 메모리 업데이트 중지: simulation_id={simulation_id}")
                except Exception as e:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        해당 actions.jsonl 파일의 존재 여부로 플랫폼 활성화 여부 판단

        Returns:
            모든 활성화된 플랫폼이 완료되면 True
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, state.simulation_id)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # agent_id가 없는 기록 건너뛰기 (비에이전트 동작)
                    if "agent_id" not in data:
                        continue
                    
                    # 플랫폼 가져오기: 기록의 platform 우선, 그렇지 않으면 기본 플랫폼 사용
    ...
    def get_all_actions(
        ...
        """
        모든 플랫폼의 전체 동작 기록 가져오기 (페이지네이션 제한 없음)
        ...
        # 플랫폼별 파일이 존재하지 않으면 이전 단일 파일 형식 읽기 시도
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        참고: 설정 파일(simulation_config.json)과 프로필 파일은 삭제되지 않음
    ...
    def cleanup_all_simulations(cls):
        ...
        서버 종료 시 호출되어 모든 하위 프로세스가 종료되도록 보장
        """
        # 중복 정리 방지
            ...
            try:
                if process.poll() is None:  # 프로세스가 여전히 실행 중
                    ...
                    if state:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = "서버 종료, 시뮬레이션이 종료됨"
                        cls._save_run_state(state)
                    
                    # state.json도 업데이트하여 상태를 stopped로 설정
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        Flask 애플리케이션 시작 시 호출되어 서버 종료 시 모든 시뮬레이션 프로세스가 정리되도록 보장
        """
        ...
        # Flask debug 모드에서 reloader 하위 프로세스에서만 클린업 등록 (실제 애플리케이션 실행 프로세스)
        # WERKZEUG_RUN_MAIN=true는 reloader 하위 프로세스를 나타냄
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # 등록됨 표시, 하위 프로세스의 재시도 방지
            return

        # 기존 시그널 핸들러 저장
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP는 Unix 시스템에만 존재 (macOS/Linux), Windows에는 없음
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        """
        시뮬레이션 환경이 활성 상태인지 확인 (Interview 명령 수신 가능)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: 지정 플랫폼 (선택사항)
                ...
                - None: 이중 플랫폼 시뮬레이션 시 두 플랫폼 동시 인터뷰, 통합 결과 반환
            timeout: 타임아웃 (초)
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: 시뮬레이션 ID
            interviews: 인터뷰 목록, 각 요소는 {"agent_id": int, "prompt": str, "platform": str(선택사항)} 포함
            platform: 기본 플랫폼 (선택사항, 각 인터뷰 항목의 platform으로 덮어씀)
                - "twitter": 기본적으로 Twitter 플랫폼만 인터뷰
                - "reddit": 기본적으로 Reddit 플랫폼만 인터뷰
                - None: 이중 플랫폼 시뮬레이션 시 각 에이전트를 두 플랫폼에서 동시 인터뷰
            timeout: 타임아웃 (초)
    ...
    def interview_all_agents(
        ...
        동일한 질문으로 시뮬레이션의 모든 에이전트를 인터뷰

        Args:
            simulation_id: 시뮬레이션 ID
            prompt: 인터뷰 질문 (모든 에이전트가 동일한 질문 사용)
            platform: 지정 플랫폼 (선택사항)
                - "twitter": Twitter 플랫폼만 인터뷰
                - "reddit": Reddit 플랫폼만 인터뷰
                - None: 이중 플랫폼 시뮬레이션 시 각 에이전트를 두 플랫폼에서 동시 인터뷰
        ...
        if not agent_configs:
            raise ValueError(f"시뮬레이션 설정에 에이전트가 없음: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        시뮬레이션에 환경 종료 명령을 보내 대기 명령 모드를 정상적으로 종료
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: 지정 에이전트 ID (선택사항, 해당 에이전트의 기록만 가져오기)
            limit: 각 플랫폼당 반환 수량 제한
