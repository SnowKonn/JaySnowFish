...
class GraphBuilderService:
    ...
    def build_graph_async(
        ...
        Args:
            text: 입력 텍스트
            ontology: 온톨로지 정의 (인터페이스1의 출력에서 가져옴)
            graph_name: 그래프 이름
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # 이는 Zep SDK에서 요구하는 사용법이며, 경고는 동적 클래스 생성에서 발생하므로 안전하게 무시 가능
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # Zep API는 Field의 description이 필요, 필수 항목임
    ...
    def add_text_batches(
        ...
        """배치로 텍스트를 그래프에 추가하고, 모든 episode의 uuid 목록 반환"""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """모든 episode 처리 완료 대기 (각 episode의 processed 상태를 쿼리하여 확인)"""
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            nodes와 edges를 포함한 딕셔너리, 시간 정보, 속성 등 상세 데이터 포함
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
