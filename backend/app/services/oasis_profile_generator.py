...
class OasisProfileGenerator:
    ...
    2. 生成非常详细的人设（包括基本信息、职业经历、性格特征、交易与市场行为等）
    3. 区分个人实体和抽象群体实体
    """
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "你是金融市场参与者画像生成专家。生成详细、真实的人设用于市场预测模拟,最大程度还原已有现实情况。必须返回有效的JSON格式，所有字符串值不能包含未转义的换行符。"
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""为实体生成详细的金融市场参与者人设,最大程度还原已有现实情况。
...
1. bio: 市场身份简介，200字（如投资风格、关注板块、所属机构）
2. persona: 详细人设描述（2000字的纯文本），需包含:
   - 基本信息（年龄、职业角色、教育/从业背景、所在地）
   - 投资背景（管理资金规模、投资经验、与该市场情景的关联）
   - 性格与风险偏好（MBTI类型、风险承受度、面对盈亏的情绪反应）
   - 交易与市场行为（交易频率、偏好的资产类别与策略、对消息的反应速度、信息来源）
   - 市场观点（对当前标的/宏观因子的看多/看空立场、可能改变其判断的催化剂）
   - 独特特征（口头禅、标志性交易、关注的指标）
   - 个人记忆（人设的重要部分，要介绍这个个体与该市场情景的关联，以及其已有的持仓与操作）
3. age: 年龄数字（必须是整数）
4. gender: 性别，必须是英文: "male" 或 "female"
5. mbti: MBTI类型（如INTJ、ENFP等）
6. country: 国家（使用中文，如"中国"）
7. profession: 职业/市场角色
8. interested_topics: 关注的标的或主题数组
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""为机构/群体实体生成详细的金融机构设定,最大程度还原已有现实情况。
...
1. bio: 机构简介，200字，专业得体（如机构性质、管理规模、市场角色）
2. persona: 详细机构设定描述（2000字的纯文本），需包含:
   - 机构基本信息（正式名称、机构性质、成立背景、主要业务/职能）
   - 市场定位（机构类型、管理资产规模、目标客户、核心策略）
   - 决策与发声风格（信息发布特点、研究/政策口径、常用表述）
   - 市场行为特点（交易/配置风格、调仓频率、活跃交易时段）
   - 立场态度（对核心标的或宏观主题的官方立场、面对市场波动的处理方式）
   - 特殊说明（代表的投资者群体画像、运作习惯）
   - 机构记忆（机构人设的重要部分，要介绍这个机构与该市场情景的关联，以及其已有的持仓与操作）
3. age: 固定填30（机构账号的虚拟年龄）
4. gender: 固定填"other"（机构账号使用other表示非个人）
5. mbti: MBTI类型，用于描述机构风格，如ISTJ代表严谨保守
6. country: 国家（使用中文，如"中国"）
7. profession: 机构职能描述
8. interested_topics: 关注的标的或主题数组
