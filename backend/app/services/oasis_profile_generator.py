...
최적화 개선:
1. Zep 검색 기능을 호출하여 노드 정보를 2차로 풍부하게 함
2. 프롬프트를 최적화하여 매우 상세한 인물 설정 생성
...
class OasisProfileGenerator:
    ...
    2. 매우 상세한 인물 설정 생성 (기본 정보, 직업 경력, 성격 특성, 거래 및 시장 행동 등 포함)
    3. 개인 엔티티와 추상 그룹 엔티티 구분
    """
    ...
    def generate_profile_from_entity(
        ...
        Args:
            ...
            use_llm: LLM을 사용하여 상세 인물 설정 생성 여부
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Zep 그래프 하이브리드 검색 기능을 사용하여 엔티티 관련 풍부한 정보 획득

        Zep에는 내장 하이브리드 검색 인터페이스가 없으므로 edges와 nodes를 별도로 검색한 후 결과를 병합해야 합니다.
        효율성 향상을 위해 병렬 요청으로 동시 검색을 수행합니다.
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Zep 엣지 검색 {attempt + 1}차 실패: {str(e)[:80]}, 재시도 중...")
                        time.sleep(delay)
                        delay *= 2
                    else:
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.debug(f"Zep 노드 검색 {attempt + 1}차 실패: {str(e)[:80]}, 재시도 중...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zep 노드 검색이 {max_retries}회 시도 후에도 실패: {e}")
            return None
        ...
        try:
            ...
            logger.info(f"Zep 하이브리드 검색 완료: {entity_name}, {len(results['facts'])}개 사실, {len(results['node_summaries'])}개 관련 노드 획득")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 4. Zep 하이브리드 검색을 사용하여 더 풍부한 정보 획득
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if finish_reason == 'length':
                    logger.warning(f"LLM 출력이 잘림 (attempt {attempt+1}), 복구 시도 중...")
        ...
        logger.warning(f"LLM 인물 설정 생성 실패 ({max_attempts}회 시도): {last_error}, 규칙 기반 생성 사용")
        return self._generate_profile_rule_based(
            entity_name, entity_type, entity_summary, entity_attributes
        )
    
    def _fix_truncated_json(self, content: str) -> str:
        """잘린 JSON 복구 (출력이 max_tokens 제한으로 잘림)"""
        import re
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. 문자열 내 줄바꿈 문제 처리
            # 모든 문자열 값을 찾아 그 안의 줄바꿈 대체
            def fix_string_newlines(match):
                s = match.group(0)
                # 문자열 내의 실제 줄바꿈을 공백으로 대체
                s = s.replace('\n', ' ').replace('\r', ' ')
                # 불필요한 공백 대체
                s = re.sub(r'\s+', ' ', s)
                return s
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """시스템 프롬프트 획득"""
        base_prompt = "당신은 금융시장 참여자 프로필 생성 전문가입니다. 시장 예측 시뮬레이션용으로 상세하고 사실적인 인물 설정을 생성하여 기존 현실 상황을 최대한 재현합니다. 유효한 JSON 형식을 반환해야 하며, 모든 문자열 값에는 이스케이프되지 않은 줄바꿈이 포함될 수 없습니다."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""엔티티에 대한 상세 금융시장 참여자 인물 설정을 생성하고, 기존 현실 상황을 최대한 재현하세요.
...
1. bio: 시장 신원 소개, 200자 (예: 투자 스타일, 관심 섹터, 소속 기관)
2. persona: 상세 인물 설정 설명 (2000자 순수 텍스트), 포함 필요:
   - 기본 정보 (나이, 직업 역할, 교육/경력 배경, 위치)
   - 투자 배경 (관리 자금 규모, 투자 경험, 해당 시장 시나리오와의 연관)
   - 성격 및 위험 선호도 (MBTI 유형, 위험 감수 능력, 손익에 대한 감정 반응)
   - 거래 및 시장 행동 (거래 빈도, 선호 자산 유형 및 전략, 뉴스 반응 속도, 정보 출처)
   - 시장 관점 (현재 종목/거시 요인에 대한 롱/숏 입장, 판단을 바꿀 수 있는 촉매)
   - 독특한 특성 (말버릇, 대표적 거래, 관심 지표)
   - 개인 기억 (인물 설정의 중요 부분, 이 개인과 해당 시장 시나리오의 연관, 기존 포지션과 거래 소개)
3. age: 나이 숫자 (정수여야 함)
4. gender: 성별, 영어로: "male" 또는 "female"
5. mbti: MBTI 유형 (예: INTJ, ENFP 등)
6. country: 국가 (한국어 사용, 예: "한국")
7. profession: 직업/시장 역할
8. interested_topics: 관심 종목 또는 주제 배열

중요:
- 모든 필드 값은 문자열 또는 숫자여야 하며, 줄바꿈 사용 금지
- persona는 연결된 텍스트 설명이어야 함
- {get_language_instruction()} (gender 필드는 영어 male/female 사용 필수)
- 내용은 엔티티 정보와 일관성 유지
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """그룹/기관 엔티티의 상세 인물 설정 프롬프트 구성"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "없음"
        context_str = context[:3000] if context else "추가 컨텍스트 없음"

        return f"""기관/그룹 엔티티에 대한 상세 금융기관 설정을 생성하고, 기존 현실 상황을 최대한 재현하세요.
...
1. bio: 기관 소개, 200자, 전문적이고 적절함 (예: 기관 성격, 운용 규모, 시장 역할)
2. persona: 상세 기관 설정 설명 (2000자 순수 텍스트), 포함 필요:
   - 기관 기본 정보 (정식 명칭, 기관 성격, 설립 배경, 주요 업무/기능)
   - 시장 포지셔닝 (기관 유형, 운용 자산 규모, 대상 고객, 핵심 전략)
   - 의사결정 및 발언 스타일 (정보 발표 특성, 리서치/정책 기조, 자주 사용하는 표현)
   - 시장 행동 특성 (거래/배분 스타일, 리밸런싱 빈도, 활동 거래 시간대)
   - 입장 및 태도 (핵심 종목 또는 거시 주제에 대한 공식 입장, 시장 변동에 대한 처리 방식)
   - 특별 설명 (대표하는 투자자 그룹 프로필, 운영 습관)
   - 기관 기억 (기관 인물 설정의 중요 부분, 이 기관과 해당 시장 시나리오의 연관, 기존 포지션과 거래 소개)
3. age: 30으로 고정 (기관 계정의 가상 나이)
4. gender: "other"로 고정 (기관 계정은 비개인을 나타내기 위해 other 사용)
5. mbti: MBTI 유형, 기관 스타일 설명용, 예: ISTJ는 엄격하고 보수적
6. country: 국가 (한국어 사용, 예: "한국")
...
- persona는 연결된 텍스트 설명이어야 하며, 줄바꿈 사용 금지
- {get_language_instruction()} (gender 필드는 영어 "other" 사용 필수)
- age는 정수 30이어야 하고, gender는 문자열 "other"여야 함
- 기관 계정 발언은 신원 포지셔닝에 부합해야 함"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: 그래프 ID, 더 풍부한 컨텍스트를 얻기 위한 Zep 검색용
        ...
        logger.info(f"{total}개 Agent 인물 설정 병렬 생성 시작 (병렬 수: {parallel_count})...")
        print(f"\n{'='*60}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        logger.warning(f"[{current}/{total}] {entity.name} 대체 인물 설정 사용: {error}")
                    else:
                    ...
                    # 파일에 실시간 기록 (대체 인물 설정도 포함)
                    save_profiles_realtime()

        print(f"\n{'='*60}")
        print(f"인물 설정 생성 완료! 총 {len([p for p in profiles if p])}개 Agent 생성됨")
        print(f"{'='*60}\n")
        
        return profiles
    
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """생성된 인물 설정을 콘솔에 실시간 출력 (전체 내용, 잘리지 않음)"""
        separator = "-" * 70
        ...
        # 콘솔에만 출력 (중복 방지, logger는 전체 내용 출력 안 함)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - name: 사용자 실명
        - username: 시스템 내 사용자명
        - user_char: 상세 인물 설정 설명 (LLM 시스템 프롬프트에 주입, Agent 행동 안내)
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                # user_char: 전체 인물 설정 (bio + persona), LLM 시스템 프롬프트용
        ...
        logger.info(f"{len(profiles)}개 Twitter Profile을 {file_path}에 저장됨 (OASIS CSV 형식)")

    def _normalize_gender(self, gender: Optional[str]) -> str:
        """
        gender 필드를 OASIS 요구 영어 형식으로 표준화
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        to_reddit_format()과 일관된 형식을 사용하여 OASIS가 올바르게 읽을 수 있도록 합니다.
        user_id 필드를 반드시 포함해야 합니다. 이것이 OASIS agent_graph.get_agent() 매칭의 핵심입니다!

        필수 필드:
        - user_id: 사용자 ID (정수, initial_posts의 poster_agent_id와 매칭용)
        - username: 사용자명
        - name: 표시 이름
        - bio: 소개
        - persona: 상세 인물 설정
        ...
        logger.info(f"{len(profiles)}개 Reddit Profile을 {file_path}에 저장됨 (JSON 형식, user_id 필드 포함)")

    # 이전 메서드명을 별칭으로 유지, 하위 호환성 유지
