...
class OasisProfileGenerator:
    ...
    2. 매우 상세한 인물 설정 생성 (기본 정보, 직업 경력, 성격 특성, 소셜 미디어 행동 등 포함)
    3. 개인 엔티티와 추상 그룹 엔티티 구분
    """
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """시스템 프롬프트 획득"""
        base_prompt = "당신은 소셜 미디어 사용자 프로필 생성 전문가입니다. 여론 시뮬레이션용으로 상세하고 사실적인 인물 설정을 생성하여 기존 현실 상황을 최대한 재현합니다. 유효한 JSON 형식을 반환해야 하며, 모든 문자열 값에는 이스케이프되지 않은 줄바꿈이 포함될 수 없습니다."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""엔티티에 대한 상세 소셜 미디어 사용자 인물 설정을 생성하고, 기존 현실 상황을 최대한 재현하세요.
...
2. persona: 상세 인물 설정 설명 (2000자 순수 텍스트), 포함 필요:
   - 기본 정보 (나이, 직업, 교육 배경, 위치)
   - 인물 배경 (중요 경험, 이벤트와의 연관, 사회적 관계)
   - 성격 특성 (MBTI 유형, 핵심 성격, 감정 표현 방식)
   - 소셜 미디어 행동 (게시 빈도, 콘텐츠 선호도, 상호작용 스타일, 언어 특성)
   - 입장 및 관점 (주제에 대한 태도, 화나게/감동시킬 수 있는 콘텐츠)
   - 독특한 특성 (말버릇, 특별한 경험, 개인 취미)
   - 개인 기억 (인물 설정의 중요 부분, 이 개인과 이벤트의 연관, 이벤트에서 이 개인의 기존 행동과 반응 소개)
3. age: 나이 숫자 (정수여야 함)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""기관/그룹 엔티티에 대한 상세 소셜 미디어 계정 설정을 생성하고, 기존 현실 상황을 최대한 재현하세요.
...
1. bio: 공식 계정 소개, 200자, 전문적이고 적절함
2. persona: 상세 계정 설정 설명 (2000자 순수 텍스트), 포함 필요:
   - 기관 기본 정보 (정식 명칭, 기관 성격, 설립 배경, 주요 기능)
   - 계정 포지셔닝 (계정 유형, 대상 청중, 핵심 기능)
   - 발언 스타일 (언어 특성, 자주 사용하는 표현, 금기 주제)
   - 게시 콘텐츠 특성 (콘텐츠 유형, 게시 빈도, 활동 시간대)
   - 입장 및 태도 (핵심 주제에 대한 공식 입장, 논란에 대한 처리 방식)
   - 특별 설명 (대표하는 그룹 프로필, 운영 습관)
   - 기관 기억 (기관 인물 설정의 중요 부분, 이 기관과 이벤트의 연관, 이벤트에서 이 기관의 기존 행동과 반응 소개)
