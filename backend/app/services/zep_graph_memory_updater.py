...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        활동을 Zep으로 전송할 수 있는 텍스트 설명으로 변환

        자연어 설명 형식을 사용하여 Zep이 엔티티와 관계를 추출할 수 있도록 함
        시뮬레이션 관련 접두사를 추가하지 않아 그래프 업데이트 오해 방지
        """
        # 다른 액션 유형에 따라 다른 설명 생성
        action_descriptions = {
        ...
        # "에이전트 이름: 활동 설명" 형식으로 직접 반환, 시뮬레이션 접두사 미추가
    ...
    def _describe_repost(self) -> str:
        """게시물 리트윗 - 원본 게시물 내용과 작성자 정보 포함"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")

        if original_content and original_author:
            return f"{original_author}의 게시물을 리트윗했습니다: 「{original_content}」"
        elif original_content:
            return f"게시물을 리트윗했습니다: 「{original_content}」"
        elif original_author:
            return f"{original_author}의 게시물을 리트윗했습니다"
        return "게시물을 리트윗했습니다"
    
    def _describe_quote_post(self) -> str:
        """게시물 인용 - 원본 게시물 내용, 작성자 정보 및 인용 댓글 포함"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    시뮬레이션의 actions 로그 파일을 모니터링하여 새로운 에이전트 활동을 Zep 그래프에 실시간으로 업데이트.
    플랫폼별로 그룹화하여 BATCH_SIZE개의 활동이 누적되면 Zep에 배치로 전송.

    모든 의미 있는 행동이 Zep에 업데이트되며, action_args에 완전한 컨텍스트 정보 포함:
    - 좋아요/싫어요한 게시물 원문
    - 리트윗/인용한 게시물 원문
    - 팔로우/차단한 사용자 이름
    - 좋아요/싫어요한 댓글 원문
    ...
    # 배치 전송 크기 (각 플랫폼에서 몇 개 누적 후 전송할지)
    BATCH_SIZE = 5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            graph_id: Zep 그래프 ID
            api_key: Zep API Key (선택사항, 기본값은 설정에서 읽음)
        """
        ...
        # 플랫폼별 활동 버퍼 (각 플랫폼이 BATCH_SIZE까지 누적된 후 배치 전송)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args에 완전한 컨텍스트 정보 포함 (게시물 원문, 사용자 이름 등).
    ...
    def _worker_loop(self, locale: str = 'zh'):
        """백그라운드 워커 루프 - 플랫폼별로 활동을 배치로 Zep에 전송"""
        set_locale(locale)
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        활동을 Zep 그래프에 배치로 전송 (하나의 텍스트로 병합)
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(f"{display_name} 활동 {len(activities)}개를 그래프 {self.graph_id}에 배치 전송 성공")
                logger.debug(f"배치 내용 미리보기: {combined_text[:200]}...")
                return

            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Zep에 배치 전송 실패 (시도 {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Zep에 배치 전송 실패, {self.MAX_RETRIES}회 재시도 완료: {e}")
    ...
    def _flush_remaining(self):
        ...
        # 그런 다음 각 플랫폼 버퍼에 남은 활동 전송 (BATCH_SIZE 미만이어도)
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"{display_name} 플랫폼의 남은 활동 {len(buffer)}개 전송")
