...
핵심 검색 도구 (최적화 버전):
1. InsightForge (심층 인사이트 검색) - 가장 강력한 하이브리드 검색, 자동으로 하위 질문을 생성하고 다차원 검색
2. PanoramaSearch (광범위 검색) - 만료된 콘텐츠를 포함한 전체 뷰 획득
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"- 현재 유효 사실: {self.active_count}개",
            f"- 기록/만료 사실: {self.historical_count}개"
        ]

        # 현재 유효한 사실 (완전 출력, 절삭 없음)
        if self.active_facts:
            text_parts.append(f"\n### 【현재 유효 사실】(시뮬레이션 결과 원문)")
            for i, fact in enumerate(self.active_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")

        # 기록/만료 사실 (완전 출력, 절삭 없음)
...
class AgentInterview:
    ...
    question: str  # 인터뷰 질문
    response: str  # 인터뷰 답변
    key_quotes: List[str] = field(default_factory=list)  # 핵심 인용
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # 너무 긴 콘텐츠 절삭 (마침표 기준 절삭, 하드 절삭 아님)
...
class ZepToolsService:
    ...
    2. panorama_search - 광범위 검색 (만료 콘텐츠 포함 전체 뷰 획득)
    3. quick_search - 간단 검색 (빠른 검색)
    4. interview_agents - 심층 인터뷰 (시뮬레이션 에이전트 인터뷰, 다각도 관점 획득)
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        if not self.api_key:
            raise ValueError("ZEP_API_KEY가 설정되지 않음")

        self.client = Zep(api_key=self.api_key)
        # InsightForge의 하위 질문 생성을 위한 LLM 클라이언트
    ...
    def search_graph(
        ...
        하이브리드 검색 (시맨틱+BM25)을 사용하여 그래프에서 관련 정보 검색.
        Zep Cloud의 search API를 사용할 수 없는 경우 로컬 키워드 매칭으로 폴백.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        그래프의 모든 엣지 가져오기 (페이지��이션, 시간 정보 포함)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        그래프의 모든 엣지를 가져온 후 지정된 노드와 관련된 엣지 필터링
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # 엣지가 지정된 노드와 관련되어 있는지 확인 (소스 또는 타겟으로)
    ...
    def get_simulation_context(
        ...
        시뮬레이션 요구사항과 관련된 모든 정보 종합 검색

        Args:
            graph_id: 그래프 ID
            simulation_requirement: 시뮬레이션 요구사항 설명
            limit: 각 정보 유형당 수량 제한
    ...
    def insight_forge(
        ...
        2. 각 하위 질문에 대해 시맨틱 검색
        3. 관련 엔티티를 추출하고 상세 정보 가져오기
        4. 관계 체인 추적
        ...
        # Step 3: 엣지에서 관련 엔티티 UUID 추출, 해당 엔티티의 정보만 가져오기 (전체 노드 가져오기 아님)
        entity_uuids = set()
        ...
        # 모든 관련 엔티티의 상세 정보 가져오기 (수량 제한 없음, 완전 출력)
        entity_insights = []
        node_map = {}  # 후속 관계 체인 구성용

        for uuid in list(entity_uuids):  # 모든 엔티티 처리, 절삭 없음
            if not uuid:
                continue
            try:
                ...
                if node:
                    ...
                    entity_insights.append({
                        ...
                        "type": entity_type,
                        "summary": node.summary,
                        "related_facts": related_facts  # 완전 출력, 절삭 없음
                    })
            except Exception as e:
        ...
        relationship_chains = []
        for edge_data in all_edges:  # 모든 엣지 처리, 절삭 없음
            if isinstance(edge_data, dict):
                source_uuid = edge_data.get('source_node_uuid', '')
                target_uuid = edge_data.get('target_node_uuid', '')
    ...
    def _generate_sub_queries(
        ...
        복잡한 질문을 독립적으로 검색할 수 있는 여러 하위 질문으로 분해
        """
        system_prompt = """당신은 전문적인 질문 분석 전문가입니다. 복잡한 질문을 시뮬레이션 세계에서 독립적으로 관찰할 수 있는 여러 하위 질문으로 분해하는 것이 당신의 임무입니다.

요구사항:
1. 각 하위 질문은 시뮬레이션 세계에서 관련 에이전트 행동이나 이벤트를 찾을 수 있을 만큼 구체적이어야 합니다
2. 하위 질문은 원래 질문의 다양한 차원을 다뤄야 합니다 (예: 누가, 무엇을, 왜, 어떻게, 언제, 어디서)
...
JSON 형식의 하위 질문 목록을 반환하세요."""
    ...
    def panorama_search(
        ...
        모든 관련 콘텐츠와 기록/만료 정보를 포함한 전체 뷰 획득:
        1. 모든 관련 노드 가져오기
        2. 모든 엣지 가져오기 (만료/무효화된 것 포함)
        3. 현재 유효한 정보와 기록 정보 분류 정리

        이 도구는 이벤트의 전체 그림을 파악하고 변화 과정을 추적해야 하는 시나리오에 적합합니다.
    ...
    def interview_agents(
        ...
        1. 페르소나 파일을 자동으로 읽어 모든 시뮬레이션 에이전트 파악
        2. LLM을 사용하여 인터뷰 요구사항 분석, 가장 관련성 높은 에이전트 지능적 선택
        3. LLM을 사용하여 인터뷰 질문 생성
        4. /api/simulation/interview/batch 인터페이스를 호출하여 실제 인터뷰 (이중 플랫폼 동시 인터뷰)
        5. 모든 인터뷰 결과를 통합하여 인터뷰 보고서 생성

        【중요】이 기능은 시뮬레이션 환경이 실행 중이어야 함 (OASIS 환경이 종료되지 않음)

        【사용 시나리오】
        - 다양한 역할의 관점에서 이벤트에 대한 견해 파악 필요
        - 다양한 의견과 관점 수집 필요
        - 시뮬레이션 에이전트의 실제 답변 획득 필요 (LLM 시뮬레이션 아님)
        ...
        Args:
            ...
            custom_questions: 사용자 정의 인터뷰 질문 (선택사항, 제공되지 않으면 자동 생성)
        ...
        if not profiles:
            ...
            result.summary = "인터뷰 가능한 에이전트 페르소나 파일을 찾을 수 없음"
            return result
        
        result.total_agents = len(profiles)
        logger.info(t("console.loadedProfiles", count=len(profiles)))
        
        # Step 2: LLM을 사용하여 인터뷰할 에이전트 선택 (agent_id 목록 반환)
        ...
        # 최적화 접두사 추가, 에이전트 응답 형식 제한
        INTERVIEW_PROMPT_PREFIX = (
            "인터뷰를 받고 있습니다. 당신의 페르소나, 모든 과거 기억과 행동을 결합하여 "
            "다음 질문에 순수 텍스트로 직접 답변해 주세요.\n"
            "응답 요구사항:\n"
            "1. 자연어로 직접 답변하고, 어떤 도구도 호출하지 마세요\n"
            "2. JSON 형식이나 도구 호출 형식을 반환하지 마세요\n"
            "3. Markdown 제목 (예: #, ##, ###)을 사용하지 마세요\n"
            "4. 질문 번호순으로 하나씩 답변하고, 각 답변은 「질문X:」로 시작 (X는 질문 번호)\n"
            "5. 각 질문의 답변 사이에 빈 줄로 구분\n"
            "6. 답변에 실질적인 내용이 있어야 하며, 각 질문에 최소 2-3문장으로 답변\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: 실제 인터뷰 API 호출 (platform 미지정, 기본 이중 플랫폼 동시 인터뷰)
        try:
            # 일괄 인터뷰 목록 구성 (platform 미지정, 이중 플랫폼 인터뷰)
            interviews_request = []
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # 최적화된 prompt 사용
                    # platform 미지정, API가 twitter와 reddit 두 플랫폼 모두에서 인터뷰
                })
            
            logger.info(t("console.callingBatchInterviewApi", count=len(interviews_request)))
            
            # SimulationRunner의 일괄 인터뷰 메서드 호출 (platform 미전달, 이중 플랫폼 인터뷰)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # platform 미지정, 이중 플랫폼 인터뷰
                timeout=180.0   # 이중 플랫폼은 더 긴 타임아웃 필요
            )
            ...
            # 이중 플랫폼 모드 반환 형식: {"twitter_0": {...}, "reddit_0": {...}, "twitter_1": {...}, ...}
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # 핵심 인용 추출 (두 플랫폼의 답변에서)
                import re
                combined_responses = f"{twitter_response} {reddit_response}"

                # 응답 텍스트 정리: 표시, 번호, Markdown 등 간섭 제거
                clean_text = re.sub(r'#{1,6}\s+', '', combined_responses)
                clean_text = re.sub(r'\{[^}]*tool_name[^}]*\}', '', clean_text)
                clean_text = re.sub(r'[*_`|>~\-]{2,}', '', clean_text)
                clean_text = re.sub(r'(问题|질문)\d+[：:]\s*', '', clean_text)
                ...
                meaningful = [
                    ...
                    and not s.strip().startswith(('{', '问题', '질문'))
                ]
                meaningful.sort(key=len, reverse=True)
                key_quotes = [s + "。" for s in meaningful[:3]]

                # 전략2 (보충): 올바르게 짝지어진 중국어 따옴표 「」 안의 긴 텍스트
                if not key_quotes:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """에이전트 응답에서 JSON 도구 호출 래핑 정리, 실제 내용 추출"""
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: 선택된 에이전트의 전체 정보 목록
        ...
        system_prompt = """당신은 전문적인 인터뷰 기획 전문가입니다. 인터뷰 요구사항에 따라 시뮬레이션 에이전트 목록에서 가장 적합한 인터뷰 대상을 선택하는 것이 당신의 임무입니다.

선택 기준:
1. 에이전트의 신원/직업이 인터뷰 주제와 관련됨
2. 에이전트가 독특하거나 가치 있는 관점을 가질 수 있음
3. 다양한 관점 선택 (예: 지지자, 반대자, 중립자, 전문가 등)
4. 이벤트와 직접 관련된 역할 우선 선택

JSON 형식으로 반환:
{
    "selected_indices": [선택된 에이전트의 인덱스 목록],
    "reasoning": "선택 이유 설명"
}"""
...
선택 가능한 에이전트 목록 (총 {len(agent_summaries)}개):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

가장 적합한 인터뷰 대상 에이전트를 최대 {max_agents}개 선택하고 선택 이유를 설명해 주세요."""
        ...
        try:
            ...
            reasoning = response.get("reasoning", "관련성에 기반한 자동 선택")

            # 선택된 에이전트의 전체 정보 가져오기
            selected_agents = []
            valid_indices = []
...
3. 사실, 의견, 감정 등 여러 차원을 다룸
4. 자연스러운 언어, 실제 인터뷰처럼
5. 각 질문은 50자 이내로 간결하게
6. 직접 질문, 배경 설명이나 접두사 없이
...
3-5개의 인터뷰 질문을 생성해 주세요."""
    ...
    def _generate_interview_summary(
        ...
        quote_instruction = "인터뷰 대상자의 말을 인용할 때 따옴표 「」 사용" if get_locale() == 'zh' else 'Use quotation marks "" when quoting interviewees'
        system_prompt = f"""당신은 전문 뉴스 편집자입니다. 여러 인터뷰 대상자의 답변을 바탕으로 인터뷰 요약을 생성해 주세요.

요약 요구사항:
1. 각 측의 주요 관점 추출
2. 관점의 합의와 차이점 지적
3. 가치 있는 인용 강조
4. 객관적이고 중립적, 어느 쪽도 편들지 않음
5. 1000자 이내로 제한

형식 제약 (반드시 준수):
- 순수 텍스트 단락 사용, 다른 부분은 빈 줄로 구분
- Markdown 제목 사용하지 않음 (예: #, ##, ###)
- 구분선 사용하지 않음 (예: ---, ***)
- {quote_instruction}
- 키워드에 **굵게** 표시 가능, 다른 Markdown 문법은 사용하지 않음"""
