...
class SimulationManager:
    ...
    2. OASIS 에이전트 프로필 생성
    3. LLM을 사용하여 시뮬레이션 설정 파라미터를 지능적으로 생성
    ...
    def prepare_simulation(
        ...
        3. LLM을 사용하여 시뮬레이션 설정 파라미터를 지능적으로 생성 (시간, 활동성, 발언 빈도 등)
        4. 설정 파일 및 프로필 파일 저장
        ...
        try:
            ...
            # Zep 검색 기능을 활성화하고 더 풍부한 컨텍스트를 얻기 위해 graph_id 전달
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Zep 검색을 위한 graph_id 전달
            ...
            # Reddit은 생성 과정에서 이미 실시간으로 저장되었으며, 여기서 한 번 더 저장하여 완전성 보장
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter는 CSV 형식 사용! OASIS의 요구사항임
                generator.save_profiles(
                    profiles=profiles,
