"""
시뮬레이션 설정 지능형 생성기
LLM을 사용하여 시뮬레이션 요구사항, 문서 내용, 그래프 정보를 기반으로 상세한 시뮬레이션 매개변수를 자동 생성
전 과정 자동화 구현, 수동 매개변수 설정 불필요

단계별 생성 전략 채택, 한 번에 너무 긴 내용 생성으로 인한 실패 방지:
1. 시간 설정 생성
2. 이벤트 설정 생성
3. Agent 설정 배치 생성
4. 플랫폼 설정 생성
...
class AgentActivityConfig:
    ...
    # 감정 성향 (-1.0에서 1.0, 부정에서 긍정)
    sentiment_bias: float = 0.0

    # 입장 (특정 주제에 대한 태도)
    stance: str = "neutral"  # supportive, opposing, neutral, observer

    # 영향력 가중치 (발언이 다른 Agent에게 보일 확률 결정)
    influence_weight: float = 1.0


@dataclass
class TimeSimulationConfig:
    """시간 시뮬레이션 설정 (중국인 생활 패턴 기반)"""
    # 시뮬레이션 총 시간 (시뮬레이션 시간)
    ...
    # 피크 시간대 (저녁 19-22시, 중국인 가장 활발한 시간)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5

    # 비피크 시간대 (새벽 0-5시, 거의 활동 없음)
...
class SimulationConfigGenerator:
    ...
    최적의 시뮬레이션 매개변수 설정을 자동 생성

    단계별 생성 전략 채택:
    1. 시간 설정과 이벤트 설정 생성 (경량급)
    ...
    ENTITIES_PER_TYPE_DISPLAY = 20       # 유형별 표시할 엔티티 수
    ...
    def generate_config(
        ...
        Returns:
            SimulationParameters: 전체 시뮬레이션 매개변수
        """
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # max_tokens 미설정, LLM이 자유롭게 생성하도록
                )
                ...
                if finish_reason == 'length':
                    logger.warning(f"LLM 출력이 절단됨 (시도 {attempt+1})")
...
### 기본 원칙 (참고용, 구체적인 시장과 참여자에 따라 유연하게 조정 필요):
- 시뮬레이션 시나리오에 따라 대상 시장의 거래 시간대를 추론하세요. 아래는 미국 주식시장(미국 동부시간) 참고 예시입니다
- 휴장 시간대 (21시-다음날 3시) 거의 거래 활동 없음 (활성도 계수 0.05)
- 장 시작 전 4-8시 밤사이 뉴스 소화, 유동성 낮음 (활성도 계수 0.4)
- 개장 9-11시 피크 시간, 가격 발견 집중 (활성도 계수 1.5)
- 장중 12-14시 중간 수준 활발 (활성도 계수 0.7)
- 마감 15-16시 피크 시간, 종가 부근 포지션 조정 (활성도 계수 1.4)
- 장 마감 후 17-20시 실적 및 밤사이 뉴스 주목 (활성도 계수 0.5)
- **중요**: 아래 예시 값은 참고용이며, 대상 시장과 참여자 유형에 따라 구체적인 시간대를 조정해야 함
  - 예: 한국 주식 거래 시간은 9:00-15:30; 외환/암호화폐 시장은 거의 24시간 연속 거래
  - 예: 중요 거시 이벤트 (중앙은행 결정, CPI 데이터)는 장전/장후 활성도를 크게 높일 수 있음
...
{{
    ...
    "reasoning": "해당 시장 시나리오에 대한 거래 시간대 설정 설명"
}}

필드 설명:
- total_simulation_hours (int): 총 시뮬레이션 시간, 24-168시간, 돌발 이벤트는 짧게, 추세성 이벤트는 길게
...
- peak_hours (int 배열): 거래 피크 시간대 (개장/마감), 대상 시장에 따라 조정
- off_peak_hours (int 배열): 휴장 시간대, 거의 거래 활동 없음
- morning_hours (int 배열): 장 시작 전 시간대, 밤사이 뉴스 소화
- work_hours (int 배열): 장중 및 장 마감 후 시간대
- reasoning (string): 이 설정이 왜 적합한지 간략히 설명"""

        system_prompt = "당신은 금융시장 예측 시뮬레이션 전문가입니다. 순수 JSON 형식으로 반환하며, 시간 설정은 대상 시장의 거래 시간대 특성에 맞아야 합니다."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "morning_hours": [4, 5, 6, 7, 8],
            "work_hours": [11, 12, 13, 14, 17, 18, 19, 20],
            "reasoning": "기본 미국 주식 거래 시간대 설정 사용 (라운드당 1시간)"
        }
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # 휴장 시 거의 거래 없음
...
## 작업
이벤트 설정 JSON을 생성하세요:
- 시장이 관심 갖는 핵심 주제 추출 (종목, 거시 요인, 촉매 등)
- 시장 흐름 및 심리 발전 방향 설명
- 초기 시장 신호/메시지 설계, **각 메시지에 poster_type(발행자 유형)을 반드시 지정**

**중요**: poster_type은 위의 "사용 가능한 엔티티 유형"에서 선택해야 합니다. 그래야 초기 메시지가 적합한 Agent에게 할당되어 발행됩니다.
예: 중앙은행 정책 발표는 CentralBank/CentralBanker가 발행, 리서치 보고서는 SellSideAnalyst/InvestmentBank가 발행, 실적은 ListedCompany가 발행, 개인투자자 의견은 RetailInvestor가 발행.

JSON 형식으로 반환 (마크다운 금지):
{{
    "hot_topics": ["핵심주제1", "핵심주제2", ...],
    "narrative_direction": "<시장 흐름 및 심리 발전 방향 설명>",
    "initial_posts": [
        {{"content": "시장 신호/메시지 내용", "poster_type": "엔티티 유형 (사용 가능한 유형에서 선택)"}},
        ...
    ],
    "reasoning": "<간략한 설명>"
}}"""

        system_prompt = "당신은 금융시장 분석 전문가입니다. 순수 JSON 형식으로 반환하세요. poster_type은 사용 가능한 엔티티 유형과 정확히 일치해야 합니다."
    ...
    def _assign_initial_post_agents(
        ...
        각 게시물의 poster_type에 맞는 가장 적합한 agent_id 매칭
        """
        ...
        # 각 유형별 이미 사용된 agent 인덱스 기록, 동일 agent 중복 사용 방지
        ...
        for post in event_config.initial_posts:
            ...
            # 3. 아직 찾지 못한 경우, 영향력이 가장 높은 agent 사용
            if matched_agent_id is None:
                logger.warning(f"유형 '{poster_type}'에 대한 매칭 Agent를 찾지 못함, 영향력이 가장 높은 Agent 사용")
                if agent_configs:
    ...
    def _generate_agent_configs_batch(
        ...
        prompt = f"""다음 정보를 기반으로 각 엔티티에 대한 시장 참여 활동 설정을 생성하세요.
...
- **시간은 대상 시장 거래 시간대에 맞춰야 함**: 아래는 참고용(미국 동부시간), 시뮬레이션 시나리오에 따라 조정
- **규제기관/중앙은행** (RegulatoryAgency/CentralBank): 활성도 낮음(0.1-0.3), 업무 시간(9-17) 발언, 응답 느림(60-240분), 영향력 매우 높음(2.5-3.0)
- **금융 미디어** (FinancialMediaOutlet): 활성도 중간(0.4-0.6), 종일 활동(4-20), 응답 빠름(5-30분), 영향력 높음(2.0-2.5)
- **개인투자자** (RetailInvestor/Person): 활성도 높음(0.6-0.9), 개장/마감 집중(9-11,15-16), 응답 빠름(1-15분), 영향력 낮음(0.8-1.2)
- **기관 트레이더/펀드 매니저/셀사이드 애널리스트**: 활성도 중간(0.4-0.6), 영향력 중상(1.5-2.0)
...
{{
    "agent_configs": [
        {{
            ...
            "posts_per_hour": <게시/코멘트 빈도>,
            "comments_per_hour": <댓글/응답 빈도>,
            "active_hours": [<활성 시간 목록, 대상 시장 거래 시간대 고려>],
        ...
        system_prompt = "당신은 금융시장 참여자 행동 분석 전문가입니다. 순수 JSON으로 반환하며, 설정은 대상 시장의 거래 시간대 특성에 맞아야 합니다."
        ...
        try:
            ...
            logger.warning(f"Agent 설정 배치 LLM 생성 실패: {e}, 규칙 생성 사용")
            llm_configs = {}
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """규칙 기반 단일 Agent 설정 생성 (중국인 생활 패턴)"""
