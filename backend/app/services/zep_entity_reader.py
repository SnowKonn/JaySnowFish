...
Zep 그래프에서 노드를 읽고, 사전 정의된 엔티티 유형에 맞는 노드를 필터링합니다.
"""
...
class ZepEntityReader:
    ...
    2. 사전 정의된 엔티티 유형에 맞는 노드 필터링 (Labels가 Entity만 있지 않은 노드)
    3. 각 엔티티의 관련 엣지 및 연결된 노드 정보 가져오기
    """
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: 최대 재시도 횟수 (기본 3회, 최대 3회 시도)
    ...
    def filter_defined_entities(
        ...
        필터링 로직:
        - 노드의 Labels가 "Entity"만 있으면, 이 엔티티는 사전 정의된 유형에 맞지 않으므로 건너뜀
        - 노드의 Labels에 "Entity"와 "Node" 외의 라벨이 포함되어 있으면, 사전 정의된 유형에 맞으므로 유지

        Args:
            graph_id: 그래프 ID
            defined_entity_types: 사전 정의된 엔티티 유형 목록 (선택사항, 제공되면 해당 유형만 유지)
    ...
    def get_entity_with_context(
        ...
        """
        단일 엔티티와 전체 컨텍스트 가져오기 (엣지와 연결된 노드, 재시도 메커니즘 포함)
