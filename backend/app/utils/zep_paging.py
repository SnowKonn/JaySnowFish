...
Zep의 node/edge 목록 인터페이스는 UUID cursor 페이징을 사용하며,
본 모듈은 자동 페이지 넘김 로직(단일 페이지 재시도 포함)을 캡슐화하여
호출자에게 투명하게 완전한 목록을 반환합니다.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """단일 페이지 요청, 실패 시 지수 백오프 재시도. 네트워크/IO 일시적 오류만 재시도."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """페이징으로 그래프 노드 조회, 최대 max_items개 반환 (기본값 2000). 각 페이지 요청에 재시도 포함."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """페이징으로 그래프의 모든 엣지 조회, 전체 목록 반환. 각 페이지 요청에 재시도 포함."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
