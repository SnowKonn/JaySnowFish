            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"함수 {func.__name__}이 {max_retries}회 재시도 후에도 실패: {str(e)}")
                        raise
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"비동기 함수 {func.__name__}이 {max_retries}회 재시도 후에도 실패: {str(e)}")
                        raise
    ...
    def call_batch_with_retry(
        ...
        for idx, item in enumerate(items):
            try:
                ...
                logger.error(f"{idx + 1}번째 항목 처리 실���: {str(e)}")
                failures.append({
                    "index": idx,
                    "item": item,
                    "error": str(e)
